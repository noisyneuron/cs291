...
function createStairs() {
	...
	for(var i=0; i<6; i++ ) {
		...
		stepMesh.position.y = (i+1) * (stepThickness/2 + verticalStepHeight) + i * stepThickness/2 ;
		// Push step forward by half the depth, minus half the vertical step's thickness
		stepMesh.position.z = horizontalStepDepth/2 - stepThickness/2 + i * (horizontalStepDepth - stepThickness);
		scene.add( stepMesh );
	}
}
